...
vuln_map <- ggplot() +
  geom_raster(data = orfig5rast, aes(fill = orv)) +
  scale_fill_gradient(
    low = "#FFFF75",
    high = "#CF4611",
    breaks = c(0, 100),
  ...
  labs(title = "Your Vulnerability Study Title") +
  theme_minimal() +
  theme(
    legend.title = element_blank(),
    axis.title.x = element_blank(),
...
```
```{r vulnerability-difference-map}
#test to change to tmap package 
...
# use grey for lat / long labels
# switch to using tmap package
# Convert your raster to a SpatialGridDataFrame
vulnerability_sgdf <- as(vulnerability_r, "SpatialGridDataFrame")

# Create a tmap object with tm_raster
vuln_diff_map_tmap <- tm_shape(vulnerability_r) +
  tm_raster(col = "diffv", palette = "PuOr", style = "cat",
            breaks = c(-range, 0, range), 
            title = "Vulnerability:\nReproduction - Original") +
  tm_layout(title = "Vulnerability Difference Map", 
            frame = FALSE, 
            inner.margins = 0, 
