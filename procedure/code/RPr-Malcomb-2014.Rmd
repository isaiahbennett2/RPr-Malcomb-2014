...
author: "Joseph Holler, Kufre Udoh, Drew An-Pham, Middlebury Open GIScience Classes, Isaiah Bennett"
date: "`r Sys.Date()`"
...
names(capacity_r) <- "Adaptive Capacity"
plot(capacity_r)
```
...
names(drw) <- "Drought Exposure" # rename band to acronym for drought exposure
plot(drw)
```
...
names(flw) <- "Flood Risk" # rename band to acronym for flood risk
plot(flw)
```
...
color_palette <- (colorRampPalette(c( "lemonchiffon1", "lightcoral"))(100))

# Create a tmap object
tm_lhz <- tm_shape(lhz_v) +
  tm_layout(frame = FALSE, legend.position = c("right", "0.5"))

# Set the inverted color palette for sensitivity
tm_lhz <- tm_lhz + tm_fill("sensitivity", palette = color_palette, title = "Livelihood\nSensitivity\nScores")
...
# Visualize the raster using tmap
tm_shape(orfig5rast) +
  tm_raster(col = "orv") +
  tm_layout(legend.show = TRUE, legend.position = c("right", "top")) # Position the legend on the right
```
...
## Create Vulnerability Score with Geometric Aggregation 
Malcomb et al. 2014 originally used a simple additive aggregation to calculate the vulnerability index, however, this form of aggregation is not optimized for reducing uncertainty. Because of its substitutive nature, additive weighting is not sensitive to outliers. A high weight from one vulnerability indicator could mask a low weight in another, making it seem as if the overall vulnerability is low. These outliers are of particular importance in a vulnerability index such as this one to be able to identify areas of high and low priority. Geometric aggregation reduces this potential for uncertainty through its higher sensitivity to outliers. 
...
vulnerability_ga <- (40 - capacity_r) * sensitivity_r * drought_r * flood_r 
names(vulnerability_ga) <- "Vulnerability (Geometric Aggregation)"
plot(vulnerability_ga)
```
 
## Figure 6: Geometric Vulnerability 

```{r find break & limit values for geometric vulnerability map}
g_vuln_max <- max(vulnerability_ga[[1]],na.rm = TRUE)
g_vuln_min <- min(vulnerability_ga[[1]],na.rm = TRUE)
```

```{r geometric-vulnerability-map}
g_vuln_map = ggplot() +
  geom_stars(data = vulnerability_ga) +
  scale_fill_gradient(
    low = "#FFFF75",
    high = "#CF4611",
    breaks = c(g_vuln_min, g_vuln_max),
    labels = c("Lower Vulnerability", "Higher Vulnerability"),
    #labels = c(g_vuln_min, g_vuln_max),
    na.value = "transparent",
    guide = "colourbar",
...
Despite being portrayed as an easily replicable study, this study was neither easily nor fully successfully replicated. Vulnerability model studies present many opportunities for uncertainty to be hiding in some of the research design decisions due to a great amount of subjectivity in certain aspects of the research design such as choosing indicators, normalization, weighting, and aggregation. While the potential for uncertainty in these steps of any vulnerability study is common and can be accounted for, a lack of transparency around the subjectivity from these decisions in the Malcomb et al. study increasingly made it difficult to reproduce an exact replica of the original results.

Here are a few of the key areas in which the most uncertainty was encountered:

First and foremost there is the ontologically rooted uncertainty in how the original authors inconsistently define "vulnerability", "resilience", and "adaptive capacity". Depending on context, both "vulnerability" and "resilience" may refer only to the assets and access portion of the model or to the full model
inclusive of adaptive capacity (assets and access), livelihood sensitivity, and exposure. There are also strong directionality issues with their use of these terms. In Figure 5. of the original study they title it Malawi Vulnerability to Climate Change defining the function Assets+Access+Livelihoods-Exposure, however the value of each factor is unclear therefore it is unclear whether the map is actually displaying vulnerability or perhaps resilience or adaptive capacity.  

The data normalization process described in the original study is highly confusing. The original study describes how the data was quantiled into 5 classes with values from 0 to 5, however, a scale with a minimum of 0 and a maximum of 5 indicates 6 classes rather than 5. Even under the assumption that each indicator should be normalized into 5 classes, certain indicators had data that were not compatible with such a distribution. Four variables were described as (Y/N) nominal data (T1), for which it is not clear how to transform the data into ordinal data with more than two classes. A similar concern arises for the market access variable with three classes (Rural, Peri-urban, and
Urban). 

While the original study boasts "expert interviews" that back the weighting process, the interview process was completely blackboxed, making it difficult to externally validate the weighting process. While all weights were clearly given, inconsistencies in the scale of the flooding risk data that were not proportionally reflected in the weights indicated a large potential for creating uncertainty in the outcome of the study. This uncertainty in the flood data is aleatoric in the low-resolution of the raster compared to all of the other data sources. Each raster cell in the flood data has a considerably larger area than the drought data indicating an over-generalization of the true representation of which areas are vulnerable to floods. Despite its worse resolution, the drought risk and flooding risk are still weighted the same at 20%. This error of uncertainty was not accounted for in the original study, and still presents an opportunity to improve the reanalysis by either decreasing the weight or replacing it with data that has an equal resolution to the drought risk. 

The original study was also unclear in how it combined weights and executed the final aggregation. The first reproduction of this study assumed an additive aggregation where values have no direct influence on each other. This current reanalysis notes the insensitivity of additive aggregation to outliers that are particularly valuable to vulnerability models and thus proposes a new geometric aggregation to prioritize the influence of these outliers that indicate areas of considerable high vulnerability. 

Ultimately, the reproduction of this study demonstrates that reproducibility remains an immense challenge even for a study that claims to be replicable when there remains an absence of complete transparency in research design.    
